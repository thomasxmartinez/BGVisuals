...
const ThreeCubeDance: React.FC<ThreeCubeDanceProps> = ({ audioFeatures, isPlaying, currentTime: _currentTime }) => {
  ...
  const cleanup = useCallback(() => {
    console.log('🎯 ThreeCubeDance cleaning up...');
    
    // Kill all GSAP tweens
    gsapTweensRef.current.forEach(tween => {
    ...
    // Dispose all meshes and geometries/materials
    if (sceneRef.current) {
      sceneRef.current.traverse((obj) => {
        ...
        if ((obj as THREE.Mesh).material) {
          ...
          if (Array.isArray(mat)) {
            (mat as THREE.Material[]).forEach((m: THREE.Material) => {
              if (typeof m.dispose === 'function') m.dispose();
            });
          } else {
            if (typeof (mat as THREE.Material).dispose === 'function') (mat as THREE.Material).dispose();
          }
        }
      });
      // Remove all children
  ...
  const init = useCallback(() => {
    ...
    for (let i = 0; i < gridSize; i++) {
      for (let j = 0; j < gridSize; j++) {
        ...
        if (headTexture) {
          ...
          box = new THREE.Mesh(geometry, specialMaterial);
    ...
    const animate = () => {
      ...
      if (isPlaying && (energy > 0.01 || volume > 0.01)) {
        // Smoother, more elegant camera movement
        if (!isUserInteractingRef.current) {
          const cameraDistance = 30 + energy * 6; // More subtle movement
          camera.position.y = cameraDistance * 0.7;
          camera.position.z = cameraDistance;
        }
        
        // Very subtle camera rotation on beat - only if not interacting
        if (beat && !isUserInteractingRef.current) {
          camera.rotation.y += 0.008; // Much more subtle rotation
        }
        
        // Very subtle light pulsing on beat
        if (beat) {
          light.intensity = 2.05;
          light1.intensity = 2.05;
          light2.intensity = 2.05;
        } else {
          light.intensity = 2;
          light1.intensity = 2;
          light2.intensity = 2;
        ...
        // More refined cube reactivity to music
        const allCubes = [...oddsRef.current, ...evensRef.current];
        allCubes.forEach((cube, index) => {
          const baseScale = cube.userData.originalScale || 1.0;
          const scale = baseScale + volume * 0.15; // Much more subtle scaling
          cube.scale.setScalar(scale);
          
          // Special behavior for head cubes
          if (cube.userData.isHeadCube) {
            // More refined music reactivity for head cubes
            if (beat) {
              // Elegant beat animation for head cubes
              cube.rotation.y += 0.08; // Slower, more elegant rotation
              cube.rotation.x += 0.04;
              cube.rotation.z += 0.03;
              
              // Subtle pulse effect on beat
              const pulseScale = baseScale * 1.2 + volume * 0.2;
              cube.scale.setScalar(pulseScale);
              
              // Refined glow on beat
              (cube.material as THREE.MeshPhongMaterial).emissiveIntensity = 0.4;
              ...
              // Very gentle continuous rotation for head cubes
              cube.rotation.y += 0.01; // Much slower rotation
              (cube.material as THREE.MeshPhongMaterial).emissiveIntensity = 0.15;
              if (cube.userData.isRedCube) {
                (cube.material as THREE.MeshPhongMaterial).emissive.setHex(0xff4444);
              } else {
                (cube.material as THREE.MeshPhongMaterial).emissive.setHex(0x4444ff);
            ...
            // More subtle floating motion for head cubes
            const time = Date.now() * 0.0005; // Slower floating
            cube.position.y = Math.sin(time + index * 0.3) * 0.2; // Smaller amplitude
          } else {
            // Very subtle regular cube behavior
            if (beat) {
              cube.rotation.x += 0.02; // Much slower rotation
  ...
  const animateBoxes = useCallback((list: THREE.Mesh[], callback: () => void) => {
    for (let i = 0; i < list.length; i++) {
      ...
      // Base bounce height increases with music energy - keep high but smooth
      let bounceHeight = 6.0 + energy * 6.0; // Slightly reduced but still high
      
      // Add extra height on beat - more subtle
      if (beat) {
        bounceHeight += 2.0;
      }
      
      // Add volume-based variation - more subtle
      bounceHeight += volume * 3.0;
      
      // Ensure minimum bounce for visual appeal
      bounceHeight = Math.max(bounceHeight, 4.0);
      
      // Much slower, smoother timing with elegant wave effect
      const delay = (i / list.length) * 1.2; // Slower wave spread
      const duration = 2.5 + (energy * 0.8); // Much longer, smoother duration

      const positionTween = gsap.to(element.position, {
        y: bounceHeight,
        duration: duration,
        delay: delay,
        ease: "power2.inOut", // Much smoother easing - no elastic
        yoyo: true,
        repeat: -1
      });
      gsapTweensRef.current.push(positionTween);

      // Much slower, more elegant rotation
      const rotationSpeed = 3.0 + (energy * 0.6); // Slower rotation
      const rotationTween = gsap.to(element.rotation, {
        z: `-=${Math.PI * 2}`,
        y: `+=${Math.PI * 2}`,
        duration: rotationSpeed,
        delay: delay,
        ease: "sine.inOut", // Keep smooth sine easing
        repeat: -1
      });
      gsapTweensRef.current.push(rotationTween);
    }

    // Much longer delay for more relaxed, classy animation
    const delayedCall = gsap.delayedCall(4.0, callback);
    gsapTweensRef.current.push(delayedCall);
  }, []);
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (intersects.length > 0) {
        ...
        if (clickedCube.userData.isHeadCube) {
          // Elegant animation for head cubes: smooth spin and refined glow
          const rotationTween = gsap.to(clickedCube.rotation, {
          ...
          // Use refined glow for click effect
          (clickedCube.material as THREE.MeshPhongMaterial).emissive.setHex(0xffffff);
          (clickedCube.material as THREE.MeshPhongMaterial).emissiveIntensity = 0.8;
          setTimeout(() => {
            (clickedCube.material as THREE.MeshPhongMaterial).emissive.copy(originalEmissive);
            (clickedCube.material as THREE.MeshPhongMaterial).emissiveIntensity = 0.15;
          }, 1500);
        } else {
          // Elegant bounce for regular cubes
          const positionTween = gsap.to(clickedCube.position, {
            y: clickedCube.position.y + 8, // Slightly lower bounce
